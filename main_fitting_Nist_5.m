...
%% Fitting isotherms with data from NIST databank
    ...
    switch model
        ...
        case 'toth'
            % toth isotherm (Regular Toth model with just the physical adsorption)
            T0 = 293;
                 %  Xi_p, dH_p(kJ/mol), alpha_p, ns0_p(mol/kg), b0_p (1/Pa), t0_p
            lb = [   0         1            0         0.5      1e-010      0];
            ub = [   10        50           2         18       7e-04       5];

            p0 = [   5         5          0.43        10       5e-8        0.66 ];
    ...
    function yp = toth(xm,R,T0,T,lb,ub)
        yp = @(p) (((p(4)*(ub(4)-lb(4))+lb(4))*exp((p(1)*(ub(1)-lb(1))+lb(1))*(1-T/T0)).*((p(5)*(ub(5)-lb(5))+lb(5))*exp((p(2)*(ub(2)-lb(2))+lb(2))/(R*T0)*(T0./T-1))).*xm)./((1+(((p(5)*(ub(5)-lb(5))+lb(5))*exp((p(2)*(ub(2)-lb(2))+lb(2))/(R*T0)*(T0./T-1))).*xm).^((p(6)*(ub(6)-lb(6))+lb(6))+(p(3)*(ub(3)-lb(3))+lb(3))*(1-T0./T))).^(1./((p(6)*(ub(6)-lb(6))+lb(6))+(p(3)*(ub(3)-lb(3))+lb(3))*(1-T0./T)))));
    end
